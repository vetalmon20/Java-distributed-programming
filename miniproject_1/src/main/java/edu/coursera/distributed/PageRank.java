...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer,Double> rank =
                sites.join(ranks).flatMapToPair(
                        kv -> {
                            Tuple2<Website, Double> currVal = kv._2();
                            Double currRank = currVal._2();
                            Website edges = currVal._1();

                            LinkedList<Tuple2<Integer, Double>> contributionsList = new LinkedList<>();
                            Iterator<Integer> iterator = edges.edgeIterator();

                            while (iterator.hasNext()) {
                                final int target = iterator.next();
                                contributionsList.add(new Tuple2(target, currRank / (double) edges.getNEdges()));
                            }

                            return contributionsList;
                        });

        return rank.reduceByKey(Double::sum).mapValues(x -> 0.15 + 0.85 * x);
    }
}
